class Sprite {
  ...
  draw() {
    // draws a sprite based on location and the sprite frame
    c.drawImage(
      this.image,
...
class Fighter extends Sprite {
  ...
  update() {
    // updates each element, draws and applies physics and borders
    this.draw();
    if (!this.isdead) {
      this.animateFrame();
    }
    ...
    if (this.position.x + this.velocity.x > 970) {
      // set border left
      this.position.x = 970;
    }
  ...
  Attacking() {
    // function that sets attacking to true for some while
  ...
  SwitchSprite(sprite) {
    // function that switches the player sprites based on input
    ...
    if (
      (this.image === this.sprites.punch.image &&
        this.framesCurrent < this.sprites.punch.framemax - 1) ||
      (this.image === this.sprites.fpunch.image &&
        this.framesCurrent < this.sprites.fpunch.framemax - 1)
    ) {
      return;
    }
